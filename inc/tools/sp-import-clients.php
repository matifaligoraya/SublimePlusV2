 ...
 * Run once, then remove or keep — it is idempotent (skips existing files).
 *
...
const SP_ASSETS_BASE = 'https://precastuae.ae';
...
function sp_import_assets_page() {
    global $sp_client_assets, $sp_supply_assets;
    ...
    if ($ran) {
        ...
        foreach ($sp_supply_assets as $filename => $path) {
    ...
    $clients_total  = count($sp_client_assets);
    $supply_total   = count($sp_supply_assets);
    $clients_done   = 0;
    $supply_done    = 0;
    foreach (array_keys($sp_client_assets) as $f) {
        if (file_exists($client_dir . $f)) $clients_done++;
    }
    foreach (array_keys($sp_supply_assets) as $f) {
        if (file_exists($supply_dir . $f)) $supply_done++;
    }
    ?>
    <div class="wrap">
      <h1>Import Site Assets</h1>
      <p>Downloads client logos and supply card images from <strong>precastuae.ae</strong> directly into the theme's <code>assets/img/</code> folder. Skips files that already exist.</p>

      <table class="widefat" style="max-width:500px;margin-bottom:1.5rem">
      ...
      <?php if ($ran && !empty($results)) : ?>
        <h3>Import log</h3>
        <ul style="max-height:300px;overflow:auto;background:#f6f7f7;padding:1rem;border:1px solid #ddd;border-radius:4px">
          <?php foreach ($results as [$status, $msg]) :
            $color = ['ok' => '#166534', 'skip' => '#92400e', 'error' => '#991b1b'][$status] ?? '#000';
          ?>
            <li style="color:<?php echo $color; ?>;margin:.25rem 0"><?php echo esc_html("[$status] $msg"); ?></li>
          <?php endforeach; ?>
        </ul>
      <?php endif; ?>
      ...
      <form method="post">
        ...
        <p>
          <button type="submit" name="sp_run_import" value="1" class="button button-primary button-large">
            <?php echo ($clients_done + $supply_done === $clients_total + $supply_total) ? 'Re-download All Assets' : 'Download Missing Assets'; ?>
          </button>
        </p>
      </form>
    </div>
