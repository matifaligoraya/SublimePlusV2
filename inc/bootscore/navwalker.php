...
if (!function_exists('bootscore_register_navwalker')) :
  function bootscore_register_navwalker() {
    ...
    if (class_exists('bootstrap_5_wp_nav_menu_walker')) return;
    ...
    class bootstrap_5_wp_nav_menu_walker extends Walker_Nav_menu {
      ...
      function start_el(&$output, $item, $depth = 0, $args = null, $id = 0) {
        ...
        // Use nav_menu_item_title so SublimePlusV2 icon hooks fire correctly
        $title = apply_filters('nav_menu_item_title', $item->title, $item, $args, $depth);

        $item_output  = $args->before;
        $item_output .= '<a' . $attributes . '>';
        $item_output .= $args->link_before . wp_kses_post($title) . $args->link_after;
        $item_output .= '</a>';
