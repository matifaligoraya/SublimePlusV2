<?php
/**
 * Sublimeplus_Customize_Sanitizer
 *
 ...
 *
 */
final class Sublimeplus_Customize_Sanitizer
{
    private $control;
    private $setting;
    private $fonts = [];
                    ...
                    if (is_array($value)) {
                        foreach (Sublimeplus_Customizer::SUPPORT_DEVICES as $device) {
                            if (isset($value[$device])) {
                                $has_device = true;
                                $value[$device] = Sublimeplus_sanitize_color_input($value[$device]);
                            }
                        }
                    }
                }
                ...
                if ($device_settings && !$this->skip_devices) {
                    if (is_array($value)) {
                        ...
                        foreach (Sublimeplus_Customizer::SUPPORT_DEVICES as $device) {
                            if (isset($value[$device])) {
                                $has_device = true;
                                $value[$device] = Sublimeplus_sanitize_checkbox_value($value[$device]);
                            }
                        }
                    }
                }
                if (!$has_device) {
                    $value = Sublimeplus_sanitize_checkbox_value($value);
                }
                ...
                if ($device_settings && !$this->skip_devices) {
                    if (is_array($value)) {
                        ...
                        foreach (Sublimeplus_Customizer::SUPPORT_DEVICES as $device) {
                            if (isset($value[$device])) {
                                $has_device = true;
                                $value[$device] = Sublimeplus_sanitize_text_input_recursive($value[$device]);
                            }
                        }
                    }
                }
                if (!$has_device) {
                    $value = Sublimeplus_sanitize_text_input_recursive($value);
                }
