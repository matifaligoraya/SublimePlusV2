<?php
/**
 * Sublimeplus_Demo_Importer
 *
 * @package SublimePulse\Core\Admin\Classes
 * @author   SublimePulse
 * @link     https://bitandbytelab.com/
 */
class Sublimeplus_Demo_Importer
{
    /**
     * Constructor
     */
    ...
    {
        $wxr_importer = new Sublimeplus_WXR_Importer();
    ...
    public function importThemeOptions($file)
    {
        ...
        if (is_dir(SUBLIMEPLUS_DIR.'inc/customize/presets')) {
            $presets = new DirectoryIterator(SUBLIMEPLUS_DIR.'inc/customize/presets');
        ...
        if (!empty($customize_data)) {
            ...
            if (!empty($customize_data['options'])) {
                ...
                foreach ($customize_data['options'] as $option_key => $option_value) {
                    $option = new Sublimeplus_Customize_Importer($wp_customize, $option_key, array('type' => 'option'));
    ...
    public function importThemeSettings($base_path)
    {
        ...
        if (file_exists($base_path . 'theme-settings.txt') && function_exists('Sublimeplus_base69_decode')) {
            $theme_settings = $wp_filesystem->get_contents($base_path . 'theme-settings.txt');
            if ($theme_settings) {
                $theme_settings = unserialize(Sublimeplus_base69_decode($theme_settings));
                update_option('sublimeplus_settings', $theme_settings, true);
            }
        }
