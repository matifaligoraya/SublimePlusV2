...
(function ($, _) {
	...
	api = zooMegaMenu = {
		init: function () {
			api.$body = $('body');
			api.$modal = $('#sublimeplus-settings');
			api.itemData = {};
			api.templates = {
				menus     : _.template($('#sublimeplus-tmpl-menus').html()),
				title     : _.template($('#sublimeplus-tmpl-title').html()),
				mega      : _.template($('#sublimeplus-tmpl-mega').html()),
				background: _.template($('#sublimeplus-tmpl-background').html()),
				content   : _.template($('#sublimeplus-tmpl-content').html())
			};
		...
		initActions: function () {
			...
			api.$body.on('click', '.sublimeplus-modal-backdrop, .sublimeplus-modal-close, .sublimeplus-button-cancel', this.closeModal);
            api.$body.on('change', 'input.sublimeplus-enable-mega-menu', function(e)
            {
			...
			api.$modal
				.on('click', '.sublimeplus-menu a', this.switchPanel)
				.on('click', '.sublimeplus-column-handle', this.resizeMegaColumn)
				.on('click', '.sublimeplus-button-save', this.saveChanges);
		},
		...
		closeModal: function () {
			api.$modal.hide().find('.sublimeplus-content').html('');
		...
		render: function () {
			// Render menu
			api.$modal.find('.sublimeplus-frame-menu .sublimeplus-menu').html(api.templates.menus(api.itemData));

			var $activeMenu = api.$modal.find('.sublimeplus-menu a.active');
		...
		openSettings: function (panel) {
			var $content = api.$modal.find('.sublimeplus-frame-content .sublimeplus-content'),
				$panel = $content.children('#sublimeplus-panel-' + panel);
			...
			if ($panel.length) {
				...
				$content.children('#sublimeplus-panel-' + panel).addClass('active').siblings().removeClass('active');
			...
			// Render title
			var title = api.$modal.find('.sublimeplus-frame-menu .sublimeplus-menu a[data-panel=' + panel + ']').data('title');
			api.$modal.find('.sublimeplus-frame-title').html(api.templates.title({title: title}));
		},
		...
		resizeMegaColumn: function (e) {
			...
			var steps = ['25.00%', '33.33%', '50.00%', '66.66%', '75.00%', '100.00%'],
				$el = $(this),
				$column = $el.closest('.sublimeplus-submenu-column'),
			...
			if ($el.hasClass('sublimeplus-resizable-w')) {
		...
		initMegaColumns: function () {
			var $columns = api.$modal.find('#sublimeplus-panel-mega .sublimeplus-submenu-column'),
				defaultWidth = '25.00%';
		...
		saveChanges: function () {
			var data = $('#menu, .sublimeplus-content :input').serialize(),
				$spinner = api.$modal.find('.sublimeplus-toolbar .spinner');
