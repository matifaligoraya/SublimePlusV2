...
(function(exports, $) {
    ...
    var ZooWishlistView = (function() {
        function Constructor() {
            /** Cache DOM elements */
            this.panel = $('.sublimeplus-wishlist-panel');
        ...
        Constructor.prototype.renderPanel = function(content) {
            ...
            if (this.panel.length) {
                this.panel.find('.sublimeplus-wishlist-panel-inner').html(content);
            } else {
                $('body').append('<div class="sublimeplus-wishlist-panel sublimeplus-popup-panel" style="display:none"><div class="loading popup-mask-close"></div><button class="sublimeplus-wishlist-panel-close-btn popup-close-btn" type="button" name="close-button"><i class="sublimeplus-icon-close"></i> </button><div class="sublimeplus-wishlist-panel-inner sublimeplus-popup-inner" style="display: none">' + content + '</div></div></div>');
            }
        }
        ...
        Constructor.prototype.renderBrowseButton = function(btn) {
            ...
            btn = $(document).find('.sublimeplus-wishlist-button[data-id='+btn.attr('data-id')+']');
    ...
    var ZooWishlistController = (function() {
        ...
        function Constructor(model, view) {
            ...
            if (itemsCount < 1) {
                this.view.counter.addClass('sublimeplus-hidden');
            } else {
                this.view.counter.removeClass('sublimeplus-hidden');
            }
        ...
        Constructor.prototype.sync = function(changeEvent) {
            ...
            if ('zooWishlistItems' === changeEvent.key) {
                ...
                if (items.length < 1) {
                    $('.wishlist-counter').addClass('sublimeplus-hidden');
                }
            }
        };
            ...
            if (variationInput.length) {
                variationInput.on('change', function(changeEvent) {
                    var items = self.model.list(),
                        ...
                        btn = $(this).parent().find('.sublimeplus-wishlist-button');
            ...
            if (target.hasClass('browse-wishlist') && target.hasClass('redirect-link')) {
                ...
                if (this.model.add(itemID)) {
                    ...
                    if (target.hasClass('redirect-link')) {
                        if ($(document).find('.sublimeplus-product-popup-notice')[0]) {
                            $(document).find('.sublimeplus-product-popup-notice').addClass('active');
                        } else {
                            $('body').append('<div class="sublimeplus-product-popup-notice">' + target.data("label-added") + '</div>');
                            setTimeout(function() {
                                $(document).find('.sublimeplus-product-popup-notice').addClass('active');
                            }, 100);
                        }
                        setTimeout(function() {
                            $(document).find('.sublimeplus-product-popup-notice').removeClass('active');
                        }, 1200);
                    } else {
                        return this.browseAction(addEvent);
                    }
        ...
        Constructor.prototype.removeAction = function(removeEvent) {
            ...
            var itemID = $(removeEvent.currentTarget).attr('data-id'),
                button = $('.sublimeplus-wishlist-button[data-id="' + itemID + '"]'),
        ...
        Constructor.prototype.browseAction = function(browseEvent) {
            ...
            if (items.length && !this.doingAJAX) {
                ...
                $.ajax({
                    ...
                    self.view.panel.find('.sublimeplus-wishlist-panel-inner').fadeIn();
                });
            } else {
                self.view.panel.find('.sublimeplus-wishlist-panel-inner').fadeIn();
            }
        ...
        Constructor.prototype.listen = function() {
            ...
            this.view.panel.find('.sublimeplus-wishlist-panel-close-btn').on('click', function(closeEvent) {
